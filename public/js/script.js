...
document.addEventListener('DOMContentLoaded', function () {
    ...
    sidebarToggle.addEventListener('click', () => {
        ...
        if (window.isCalendarRendered && window.calendarInstance) {
            setTimeout(() => {
    ...
    function applyDarkMode(isDark) {
        ...
        if (window.patientAdmissionsChartInstance) {
            window.patientAdmissionsChartInstance.destroy();
            initPatientAdmissionsChart();
        }
        if (window.departmentPopularityChartInstance) {
            window.departmentPopularityChartInstance.destroy();
            initDepartmentPopularityChart();
        }
        // Re-render calendar for theme
        if (window.isCalendarRendered && window.calendarInstance && !document.getElementById('appointments').classList.contains('hidden')) {
            window.calendarInstance.destroy();
            window.isCalendarRendered = false;
            initFullCalendar();
        }
    }
    ...
    function showSection(sectionId) {
        contentSections.forEach(section => {
            if (section.id === sectionId) {
                ...
                if (sectionId === 'appointments' && !window.isCalendarRendered) {
                    initFullCalendar();
                } else if (sectionId === 'appointments' && window.calendarInstance) {
                    window.calendarInstance.updateSize();
                }
            } else {
    ...
    function initPatientAdmissionsChart() {
        ...
        window.patientAdmissionsChartInstance = new Chart(ctx1, {
            type: 'line',
            data: {
    ...
    function initDepartmentPopularityChart() {
        ...
        const colors = getChartColors();
        window.departmentPopularityChartInstance = new Chart(ctx2, {
            type: 'doughnut',
            data: {
    ...
    function initFullCalendar() {
        ...
        if (calendarEl && !window.isCalendarRendered) {
                ...
                dateClick: function (info) {
                    Swal.fire({
                        ...
                        customClass: {
                            ...
                            confirmButton: 'px-4 py-2 rounded-lg font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 bg-blue-600 hover:bg-blue-700 text-white focus:ring-blue-500',
                ...
                eventClick: function (info) {
                    Swal.fire({
                        ...
                        customClass: {
                            popup: 'dark:bg-gray-800 dark:text-white',
                            confirmButton: 'px-4 py-2 rounded-lg font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 bg-blue-600 hover:bg-blue-700 text-white focus:ring-blue-500',
                            denyButton: 'px-4 py-2 rounded-lg font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 bg-red-500 hover:bg-red-600 text-white focus:ring-red-400 ml-2',
                            cancelButton: 'px-4 py-2 rounded-lg font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 bg-gray-200 hover:bg-gray-300 text-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 focus:ring-gray-400 ml-2'
                        }, buttonsStyling: false
                    }).then((result) => {
                        if (result.isConfirmed) {
                            Swal.fire({
                                title: 'Edit Event',
                                text: 'Edit functionality placeholder.',
                                icon: 'info',
                                customClass: {
                                    popup: 'dark:bg-gray-800 dark:text-white',
                                    confirmButton: 'px-4 py-2 rounded-lg font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 bg-blue-600 hover:bg-blue-700 text-white focus:ring-blue-500'
    ...
    // --- DataTables for Patients ---
    if (window.jQuery && $('#patientTable').length) {
        $('#patientTable').DataTable({
            responsive: true,
            data: [
                ["P001", "John Doe", "35", "Male", "2025-05-10", "Dr. Smith", ""],
                ["P002", "Jane Smith", "28", "Female", "2025-05-12", "Dr. Lee", ""],
                ["P003", "Robert Brown", "45", "Male", "2025-05-08", "Dr. Wilson", ""],
                ["P004", "Emily White", "62", "Female", "2025-05-11", "Dr. Smith", ""],
                ["P005", "Michael Green", "19", "Male", "2025-05-09", "Dr. Davis", ""],
                ["P006", "Olivia Black", "50", "Female", "2025-05-13", "Dr. Lee", ""],
            ],
            columns: [
                { title: "Patient ID" },
                { title: "Name" },
                ...
                { title: "Assigned Doctor" },
                {
                    title: "Actions",
                    render: function () {
                        return `<button class="inline-flex p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-blue-500 hover:text-blue-600" title="View Details"><ion-icon name="eye-outline" class="text-lg"></ion-icon></button>
                        <button class="inline-flex p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-green-500 hover:text-green-600" title="Edit"><ion-icon name="pencil-outline" class="text-lg"></ion-icon></button>
                        <button class="inline-flex p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-red-500 hover:text-red-600" title="Delete"><ion-icon name="trash-outline" class="text-lg"></ion-icon></button>`;
                    },
                    orderable: false,
    ...
    // --- SweetAlert2: Add Patient ---
    const addPatientBtn = document.getElementById('addPatientBtn');
    if (addPatientBtn) {
        addPatientBtn.addEventListener('click', () => {
            Swal.fire({
                title: 'Add New Patient',
                html: `<input id="swal-input-name" class="swal2-input" placeholder="Full Name">
                       <input id="swal-input-age" type="number" class="swal2-input" placeholder="Age">
                       <select id="swal-input-gender" class="swal2-input">
                ...
                preConfirm: () => ({
                    ...
                    age: document.getElementById('swal-input-age').value,
                ...
                if (result.isConfirmed && result.value.name) {
                    Swal.fire({
                        title: 'Success!',
                        text: `${result.value.name} added successfully.`,
                        icon: 'success',
                        customClass: {
                            popup: 'dark:bg-gray-800 dark:text-white',
                            confirmButton: 'px-4 py-2 rounded-lg font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 bg-blue-600 hover:bg-blue-700 text-white focus:ring-blue-500'
    ...
    if (createInvoiceBtn) {
        createInvoiceBtn.addEventListener('click', () => {
            Swal.fire({
                ...
                position: 'top-end',
                icon: 'info',
                title: 'Create Invoice form would appear here.',
                showConfirmButton: false,
                timer: 3000,
                timerProgressBar: true,
                customClass: { popup: 'dark:bg-gray-700 dark:text-white' }
            });
        });
    }

    // --- SweetAlert2: Upload Medical Record ---
